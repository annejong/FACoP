...
#OLD: blastColumns=['locus_tag', 'sp','shortName', 'longName','% match','alnLength','mismatches','gaps','QueryStart','QueryEnd','UniprotStart','UniprotEnd','Evalue','Score']
...
df = pd.merge(diamond, db,  how='inner', on='longName')[['locus_tag','longName','classes','description']]
...
for index, row in df.iterrows():
	for CLASS in row['classes'].split(';'):
		try:
			dict[i] = {'locus_tag': row['locus_tag'], 'class': CLASS, 'description': class_description.loc[CLASS]['description']}
			i+=1
			pass
		except KeyError:
			pass
...
cols=['locus_tag','class','description']  # select and re-order columns for export
result[cols].sort_values(by='locus_tag').to_csv(args.outfile, sep='\t', index=False)	
